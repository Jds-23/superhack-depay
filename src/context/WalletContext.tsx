...
} from '@/lib/types/WalletContextTypes';
import {
  ConnectErrorType,
...
const WalletContextProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const handleEvmConnectionChanges = useCallback(
    async (connections: GetConnectionsReturnType | undefined) => {
      ...
      if (!connections) {
        ...
        setConnectedAccounts((curr) => ({ ...curr, evm: {} }));
      ...
      setConnectedAccounts((curr) => {
        return { ...curr, ..._connectedAccounts };
      });
  ...
  //     setConnectedAccounts({ ...connectedAccounts, ..._connectedAccounts });
    ...
    async (walletId: WalletId) => {
      ...
      if (wallet.type === NetworkType.EVM) {
        setConnectedWallets((v) => ({
          ...v,
          evm: { ...v.evm, [walletId]: { address: v[wallet.type][walletId]?.address, loading: true } },
        }));

        const connector = connectors.find((c: any) => c.id === walletId);
        if (!connector) {
        ...
        if (connector.isAuthorized && (await connector.isAuthorized())) {
          ...
          if (accounts.length === 0) {
            ...
            setConnectedWallets((v) => ({ ...v, evm: { ...v.evm, [walletId]: { address: '', loading: false } } }));
      ...
      //   setConnectedWallets((v) => ({
      //     ...v,
      //     tron: { ...v.tron, [walletId]: { address: v[wallet.type][walletId]?.address, loading: true } },
      //   }));
  ...
  const handleWalletDisconnect = useCallback(
    async (walletId: WalletId) => {
      ...
      console.log(_connectedWallets, _connectedAccounts);
  ...
  const switchChain = useCallback(
    async (chainId: string) => {
      ...
      if (currentWallet?.type === NetworkType.EVM) {
        const connector = connectors.find((c: any) => c.id === currentWallet.id);
        if (!connector) {
        ...
        if (connector.switchChain) {
          await connector.switchChain({ chainId: Number(chainId) }).catch((error: any) => {
            console.error('[Wallet] Error switching chain', error);
          });
        } else {
          console.error('[Wallet] Connector does not support chain switching');

          await connector.connect({ chainId: Number(chainId) }).catch((error: any) => {
