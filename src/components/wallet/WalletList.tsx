  ...
  return (
    ...
    >
      <Image
        ...
        alt=''
        src={getWallet(walletId as WalletId)?.icon || ''}
        width={128}
        height={128}
      />
      ...
      {showNetwork && walletData?.chainId && (
        <span className='rounded-xs border border-border1 px-2 py-1 font-mono text-xs'>
          {CHAINS_DATA[walletData.chainId as ChainIds]?.name ?? walletData.chainId}
        </span>
      )}

      {walletData?.address && (
        <span className='rounded-xs rounded-xl px-2 py-1 font-mono text-xs'>
